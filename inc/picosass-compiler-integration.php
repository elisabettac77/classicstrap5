...
add_action( 'wp_head', function  () {
    ...
    //preload theme scss
    $theme_filenames_str = "_bootstrap-loader _custom _classicstrap _theme_variables _woocommerce _wp_basic_styles main ninjabootstrap/_borders ninjabootstrap/_letter-spacing ninjabootstrap/_positioning ninjabootstrap/_sizing ninjabootstrap/_theme_colors_shades ninjabootstrap/_variables";
...
add_action( 'wp_footer', function  () {
    ...
    ?>
		<script>
			...
			document.querySelector("#classicstrap-styles-css").classList.add("classicstrap-provisional-css");
		...
		<script>
			...
			function compilingSassFinishedCallback(compiled) { 
				...
				if (lastCssBundle!=compiled.css) {
					...
					formdata.append("nonce", "<?php echo wp_create_nonce("classicstrap_save_css_bundle") ?>");
					formdata.append("action", "classicstrap_save_css_bundle");
...
add_action("wp_ajax_classicstrap_save_css_bundle", function (){
    
	//exit if unlogged or non admin
	if(!is_user_logged_in() OR !current_user_can("administrator")  ) return; 
	
    //check nonce
    check_ajax_referer('classicstrap_save_css_bundle', 'nonce');
	...
	//SAVE THE FILE
	$saving_operation = $wp_filesystem->put_contents( get_stylesheet_directory() . '/' . classicstrap_get_css_optional_subfolder_name() . classicstrap_get_complete_css_filename(), $compiled_css, FS_CHMOD_FILE ); // , 0644 ?
...
function ps_add_toolbar_items($admin_bar) {
	...
	if (!isset($_GET['autorecompile'])) {
		...
		$wp_admin_bar->add_node(array(
			...
			'title' => '<span id="icon-classicstrap-sass"></span>' . __(' Compiler', 'classicstrap'),
		...
		$wp_admin_bar->add_node(array(
			'id' => 'ps-recompile-sass', 
			'title' => '<span id="icon-classicstrap-sass"></span>' . __('Stop Compiler', 'classicstrap'),
			'href' => add_query_arg(array(
					'compile_sass' => FALSE,
					'sass_nocache'=> FALSE,
					'autorecompile'=> FALSE,
...
function ps_add_backend_toolbar_items($admin_bar) {
    ...
    $admin_bar->add_node([
        ...
        'title' => '<span id="icon-classicstrap-sass"></span>' . __('Recompile SASS', 'your-textdomain'),
