...
add_filter( 'body_class', 'classicstrap_config_body_classes' );
function classicstrap_config_body_classes( $classes ) {

	//if we are using LC's custom header, don't add anything
	if (function_exists('lc_custom_header')) return $classes; 
	
	$classes[]="classicstrap_header_navbar_position_".get_theme_mod('classicstrap_header_navbar_position');
	$classes[]="classicstrap_header_navbar_color_choice_".get_theme_mod('classicstrap_header_navbar_color_choice');

    if (get_theme_mod("enable_topbar")) $classes[]="classicstrap_topbar_enabled";
	
	return $classes;
}

//REMOVE BODY MARGIN-TOP GIVEN BY WORDPRESS ADMIN BAR //////////////////////////////////////////////////////////////////////////////////////////////////////////
add_action('get_header', 'classicstrap_filter_head');
function classicstrap_filter_head() {
	if (get_theme_mod('classicstrap_header_navbar_position')=="fixed-top") remove_action('wp_head', '_admin_bar_bump_cb');
}


// ADD CUSTOM JS & CSS TO CUSTOMIZER //////////////////////////////////////////////////////////////////////////////////////////////////////////
add_action( 'customize_controls_enqueue_scripts', 'classicstrap_customize_enqueue' );
function classicstrap_customize_enqueue() {
	...
	wp_localize_script(
		'custom-customize',
		'classicstrap_ajax_obj',
		array(
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'nonce'    => wp_create_nonce( 'classicstrap_save_css_bundle' ),
		)
	);
...
add_action("customize_register","classicstrap_theme_customize_register");
	
function classicstrap_theme_customize_register($wp_customize) {

	//COLORS section is already built,
    // so we dont need to define it

	$wp_customize->add_section("typography", array(
        "title" => __("Typography", 'classicstrap5'),
        "priority" => 50,
    ));
	 
	$wp_customize->add_section("components", array(
        "title" => __("Global Options", 'classicstrap5'),
        "priority" => 50,
    ));
	
	$wp_customize->add_section("buttons-forms", array(
        "title" => __("Forms", 'classicstrap5'),
        "priority" => 50,
    ));

	$wp_customize->add_section("buttons", array(
        "title" => __("Buttons", 'classicstrap5'),
        "priority" => 50,
    ));

    $wp_customize->add_section("navbars", array(
        "title" => __("Navbars", 'classicstrap5'),
        "priority" => 50,
    ));
	
	//istantiate  all controls needed for controlling the SCSS variables
	foreach(classicstrap_get_scss_variables_array() as $section_slug => $section_data):
		...
		foreach($section_data as $variable_name => $variable_props):
			...
			if($variable_type=="color"):
				...
				$wp_customize->add_control(
					...
					array(
						'label' => __( $variable_pretty_format_name, 'classicstrap5' ), //set the label to appear in the Customizer
			...
			if($variable_type=="text"):
				...
				$wp_customize->add_setting($variable_slug, array(
					...
					//"default" => "1rem",
					//'sanitize_callback' => 'classicstrap_sanitize_rem'
				));
				$wp_customize->add_control(new WP_Customize_Control(
					$wp_customize,
					$variable_slug,
	...
	//SANITIZE CHECKBOX
	function classicstrap_sanitize_checkbox( $input ) {		return ( ( isset( $input ) && true == $input ) ? true : false ); }

	//COLORS: ANDROID CHROME HEADER COLOR
	$wp_customize->add_setting(  'classicstrap_header_chrome_color',  array(
		...
		new WP_Customize_Color_Control(
		$wp_customize,
		'classicstrap_header_chrome_color', //give it an ID
		array(
			'label' => __( 'Header Color in Android Chrome', 'classicstrap5' ), //set the label to appear in the Customizer
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            'label' => __('Hide Tagline', 'classicstrap5'),
	...
	if (function_exists('lc_custom_header')) {
        $wp_customize->add_section("nav", array(
            "title" => __("Main Navigation Bar [Disabled]", 'classicstrap5'),
            "priority" => 60,
        ));

        // Just a message
        $wp_customize->add_setting("classicstrap_header_navbar_disabled", array(
            "default" => "",
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_header_navbar_disabled",
            array(
                'label' => __('You have enabled the LiveCanvas option to handle header, so these options are disabled.', 'classicstrap5'),
                'section' => 'nav',
            )
        ));
        ...
        $wp_customize->add_section("nav", array(
            "title" => __("Main Navigation Bar", 'classicstrap5'),
            "priority" => 60,
        ));
        
        // HEADER NAVBAR EXPAND ON BREAKPOINT
        $wp_customize->add_setting("classicstrap_header_navbar_expand", array(
            "default" => "navbar-expand-md",
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_header_navbar_expand",
            array(
                'label' => __('Navbar Expansion', 'classicstrap5'),
                'section' => 'nav',
                'type'     => 'radio',
                'description' => __('Navbar is Collapsed on mobile, and expands to a full blown menubar on chosen breakpoint', 'classicstrap5'),
                'choices'  => array(
        ...
        $wp_customize->add_setting("classicstrap_header_navbar_position", array(
            "default" => "",
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_header_navbar_position",
            array(
                'label' => __('Navbar Position', 'classicstrap5'),
                'section' => 'nav',
                'type'     => 'radio',
                'choices'  => array(
                    ''  => 'Standard Static Top',
        ...
        $wp_customize->add_setting("classicstrap_header_navbar_color_choice", array(
            'default' => 'bg-dark',
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_header_navbar_color_choice",
            array(
                'label' => __('Navbar Background Color', 'classicstrap5'),
                'section' => 'nav',
                'type'     => 'select',
                'choices'  => array(
                    'bg-primary'	=> 'Primary',	
        ...
        $wp_customize->add_setting("classicstrap_header_navbar_color_scheme_attr", array(
            'default' => 'dark',
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_header_navbar_color_scheme_attr",
            array(
                'label' => __('Color Scheme (Menubar links)', 'classicstrap5'),
                'section' => 'nav',
                'type'     => 'radio',
                'choices'  => array(
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            "enable_search_form",
            array(
                "label" => __("Enable Search Form", 'classicstrap5'),
                "section" => "nav", 
                'type'     => 'checkbox',
                )
        ));
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            array(
                "label" => __("Enable Dark Mode Switch", 'classicstrap5'),
                "section" => "nav", 
                'type'     => 'checkbox',
                )
        ));
        ...
        $wp_customize->add_section("topbar", array(
            "title" => __("Optional Topbar", 'classicstrap5'),
            "priority" => 60,
        ));
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            "enable_topbar",
            array(
                "label" => __("Enable Topbar", 'classicstrap5'),
                "description" => __("Requires Navbar position set to 'Standard static top'", 'classicstrap5'),
                "section" => "topbar", 
                'type'     => 'checkbox',
                )
        ));
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            "topbar_content",
            array(
                "label" => __("Topbar Text / HTML", 'classicstrap5'),
                "section" => "topbar",
                'type'     => 'textarea',
            )
        ));
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            array(
                'label' => __('Topbar Background Color', 'classicstrap5'),
                'section' => 'topbar',
                'type'     => 'radio',
                'choices'  => array(
                    'bg-primary'	=> 'Primary',	
        ...
        $wp_customize->add_control(new WP_Customize_Control(
            ...
            array(
                'label' => __('Topbar Text Color', 'classicstrap5'),
                'section' => 'topbar',
                'type'     => 'radio',
                'choices'  => array(
	...
	if (function_exists('lc_custom_footer')) {
        $wp_customize->add_section("footer", array(
            "title" => __("Footer [Disabled]", 'classicstrap5'),
            "priority" => 100,
        ));

        // Just a message
        $wp_customize->add_setting("classicstrap_footer_disabled", array(
            "default" => "",
            "transport" => "refresh",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_footer_disabled",
            array(
                'label' => __('You have enabled the LiveCanvas option to handle footer, so these options are disabled.', 'classicstrap5'),
                'section' => 'footer',
            )
        ));
        ...
        $wp_customize->add_section("footer", array(
            "title" => __("Footer", 'classicstrap5'),
            "priority" => 100,
        ));
        
        //FOOTER TEXT
        $wp_customize->add_setting("classicstrap_footer_text", array(
            "default" => "",
            "transport" => "postMessage",
        ));
        $wp_customize->add_control(new WP_Customize_Control(
            $wp_customize,
            "classicstrap_footer_text",
            array(
                "label" => __("Footer Text", 'classicstrap5'),
	...
	$wp_customize->add_section("singleposts", array(
        "title" => __("Single Post & Archives", 'classicstrap5'),
        "priority" => 160,
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("Hide Categories", 'classicstrap5'),
			//"description" => __("Publish and exit the Customizer to see the effect", 'classicstrap5'),
            "section" => "singleposts", 
            'type'     => 'checkbox',
			)
	));
	...
	$wp_customize->add_control(new WP_Customize_Control(
		...
		array(
			"label" => __("Hide Post Metas: Date and Author", 'classicstrap5'),
			//"description" => __("Publish and exit the Customizer to see the effect", 'classicstrap5'),
			"section" => "singleposts", 
			'type'     => 'checkbox',
			)
	...
	*/
	...
	$wp_customize->add_control(new WP_Customize_Control(
		...
		array(
			"label" => __("Hide Post Author", 'classicstrap5'),
			//"description" => __("Publish and exit the Customizer to see the effect", 'classicstrap5'),
			"section" => "singleposts", 
			'type'     => 'checkbox',
			)
	...
	$wp_customize->add_control(new WP_Customize_Control(
		...
		array(
			"label" => __("Hide Post Date", 'classicstrap5'),
			//"description" => __("Publish and exit the Customizer to see the effect", 'classicstrap5'),
			"section" => "singleposts", 
			'type'     => 'checkbox',
			)
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("Enable Sharing Buttons after the Content", 'classicstrap5'),
			"description" => __("Pure HTML only, SVG inline icons, zero bloat", 'classicstrap5'),
            "section" => "singleposts", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_section("archives", array(
        "title" => __("Archive Templates", 'classicstrap5'),
        "priority" => 160,
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "archives_template",
        array(
            "label" => __("Template", 'classicstrap5'),
            "section" => "archives",
	...
	*/
	...
	$wp_customize->add_section("addcode", array(
        "title" => __("Header / Footer Code", 'classicstrap5'),
        "priority" => 180,
    ));
	
	//ADD HEADER CODE  
	$wp_customize->add_setting("classicstrap_header_code", array(
        "default" => "",
        "transport" => "postMessage",
    ));
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_header_code",
        array(
            "label" => __("Add code to Header", 'classicstrap5'),
            "section" => "addcode",
            'type'     => 'textarea',
	...
	//ADD FOOTER CODE 
	$wp_customize->add_setting("classicstrap_footer_code", array(
        "default" => "",
        "transport" => "postMessage",
    ));
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_footer_code",
        array(
            "label" => __("Add code to Footer", 'classicstrap5'),
            "section" => "addcode",
            'type'     => 'textarea',
	...
	$wp_customize->add_setting("classicstrap_fonts_header_code", array(
        "default" => "",
		//"transport" => "refresh",
	...
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_fonts_header_code",
        array(
            "label" => __("Font Loading Header code", 'classicstrap5'),
            "section" => "addcode",
            'type'     => 'textarea',
			'description' =>__('
	...
	$wp_customize->add_setting("classicstrap_fonts_use_alternative_font_source", array(
        "default" => "",
        "transport" => "refresh",
    ));
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_fonts_use_alternative_font_source",
        array(
            "label" => __("Load Google Fonts anonymously", 'classicstrap5'),
			"description" =>  __("<b>Google Fonts can be an issue for GDPR compliance in Europe. </b>").
			__("Checking this option, Google fonts will be loaded from the privacy compliant <a target='_blank' href='https://fonts.coollabs.io/'>Coollabs Font repository</a>. ", 'classicstrap5'),
            "section" => "addcode", 
            'type'     => 'checkbox',
			)
    ));
    */
	...
	$wp_customize->add_setting("classicstrap_fonts_header_code_disable", array(
        "default" => "",
        "transport" => "refresh",
    ));
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_fonts_header_code_disable",
        array(
            "label" => __("Disable the Font Loading in Header", 'classicstrap5'),
			"description" =>  __("<b>Keep this unchecked, unless you really want. </b>").__("Prevents the code of 
			the textarea above from being served in the site's &lt;head&gt;. ", 'classicstrap5'),
            "section" => "addcode", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "body_font_object",
        array(
            "label" => __("body_font_object", 'classicstrap5'),
            "section" => "addcode",
            'type'     => 'textarea',
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("headings_font_object", 'classicstrap5'),
            "section" => "addcode",
            'type'     => 'textarea',
			'description' =>'<b>Not editable</b> - Internal purpose only.'
			)
	...
	$wp_customize->add_section("extras", array(
        "title" => __("Global Utilities", 'classicstrap5'),
        "priority" => 190,
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "disable_gutenberg",
        array(
            "label" => __("Disable the Gutenberg Content Editor", 'classicstrap5'),
			"description" => __("Disables the Gutenberg content editor on all post types. De-enqueues its CSS styles as well.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("Disable the Block-based Widgets Editor", 'classicstrap5'),
			"description" => __("Disables the Block-based Widgets Editor and restores the classic widgets editor.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("Disable WordPress Comments", 'classicstrap5'),
			"description" => __("Will completely disable the entire WP comments feature.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "disable_xml_rpc",
        array(
            "label" => __("Disable XML - RPC", 'classicstrap5'),
			"description" => __("Disabling XML-RPC will close one more door that a potential hacker may try to exploit to hack your website.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	/*
	$wp_customize->add_setting("classicstrap_disable_livereload", array(
        "default" => "",
        "transport" => "refresh",
    ));
	$wp_customize->add_control(new WP_Customize_Control(
        $wp_customize,
        "classicstrap_disable_livereload",
        array(
            "label" => __("Disable  SCSS Autocompile / LiveReload ", 'classicstrap5'),
			"description" => __("If you're not editing the SCSS files, you can check this option. Makes a difference for site admins only.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
	));
	*/
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "enable_back_to_top",
        array(
            "label" => __("Add a 'Back to Top' button to site", 'classicstrap5'),
			"description" => __("Very light implementation. To see the button, you will also need to Publish, exit the Customizer, and scroll down a long page", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        array(
            "label" => __("Open the main menu on hover", 'classicstrap5'),
			"description" => __("Very light implementation. To see the change, you will also need to Publish, exit the Customizer, and scroll down a long page", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
    ));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "enable_tooltips",
        array(
            "label" => __("Enable Tooltips & Popovers", 'classicstrap5'),
			"description" => __("Adds inline <a target='_blank' href='https://getbootstrap.com/docs/5.2/components/tooltips/#enable-tooltips'>two rows of JavaScript</a> to enable Boostrap 5 tooltips and popovers. Publish and exit the Customizer to see the change.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
	));
	...
	$wp_customize->add_control(new WP_Customize_Control(
        ...
        "disable_bootstrap",
        array(
            "label" => __("Disable Bootstrap", 'classicstrap5'),
			"description" => __("Completely disables the Bootstrap CSS and JS. You'll be responsible for adding your own CSS.", 'classicstrap5'),
            "section" => "extras", 
            'type'     => 'checkbox',
			)
	));
...
function classicstrap_get_scss_variables_array(){
    return array(
...
function classicstrap_register_main_partials( WP_Customize_Manager $wp_customize ) {
	...
	//MENUS
    //classicstrap_header_navbar_expand
	$wp_customize->selective_refresh->add_partial( 'classicstrap_header_navbar_expand', array(
        'selector' => '#wrapper-navbar',
        'settings' => array( 'classicstrap_header_navbar_expand' ),
    ...
    //classicstrap_header_navbar_expand
	$wp_customize->selective_refresh->add_partial( 'classicstrap_header_navbar_position', array(
        'selector' => '#wrapper-navbar',
        'settings' => array( 'classicstrap_header_navbar_position' ),
        'render_callback' => function() {
             return get_template_part( 'partials/header', 'navbar' ); 
        },
    ) );
    ...
    //classicstrap_header_navbar_color_choice
	$wp_customize->selective_refresh->add_partial( 'classicstrap_header_navbar_color_choice', array(
        'selector' => '#wrapper-navbar',
        'settings' => array( 'classicstrap_header_navbar_color_choice' ),
        'render_callback' => function() {
             return get_template_part( 'partials/header', 'navbar' ); 
        },
    ) );
    ...
    //classicstrap_header_navbar_color_scheme_attr
	$wp_customize->selective_refresh->add_partial( 'classicstrap_header_navbar_color_scheme_attr', array(
        'selector' => '#wrapper-navbar',
        'settings' => array( 'classicstrap_header_navbar_color_scheme_attr' ),
        'render_callback' => function() {
             return get_template_part( 'partials/header', 'navbar' ); 
        },
    ) );
	...
	*/
	...
	$wp_customize->selective_refresh->add_partial( 'footer_ending_text', array(
        ...
        'settings' => array( 'classicstrap_footer_text' ),
		'render_callback' => function() {
             return classicstrap_site_info();
        },     
    ));

	/*
	//inline css
	$wp_customize->selective_refresh->add_partial( 'classicstrap_inline_css', array(
        'selector' => '#classicstrap-inline-style',
        'settings' => array( 'classicstrap_footer_bgcolor','classicstrap_menubar_bgcolor' , 'classicstrap_links_color','classicstrap_hover_links_color','classicstrap_headings_font','classicstrap_body_font'  ),
		'render_callback' => function() {
             return classicstrap_footer_add_inline_css();
        },
    ));
	*/
	...
	*/
	...
	$wp_customize->selective_refresh->add_partial( 'enable_sharing_buttons', array(
        'selector' => '.classicstrap-sharing-buttons',
...
add_action( 'customize_register', 'classicstrap_register_main_partials' );
...
function custom_background_size( $wp_customize ) {
	...
	$wp_customize->add_control( 'background-image-size', array(
		'label'      => __( 'Background Image Size','classicstrap5' ),
		'section'    => 'background_image', 
		'priority'   => 200,
		'type' => 'radio',
		'choices' => array(
			'cover' => __( 'Cover','classicstrap5' ),
			'contain' => __( 'Contain' ,'classicstrap5'),
			'inherit' => __( 'Inherit' ,'classicstrap5'),
		)
	) );
}
