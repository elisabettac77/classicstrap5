(function($) {
	...
	function ps_save_css_bundle(){
		...
		formdata.append("nonce", classicstrap_ajax_obj.nonce);
		formdata.append("action", "classicstrap_save_css_bundle");
		formdata.append("css", theCss);
		fetch(classicstrap_ajax_obj.ajax_url, {
			method: "POST",
...
@font-face {
    ...
    function ps_update_fonts_import_code_snippet_from_object_fields() {
        ...
        $("#_customize-input-classicstrap_fonts_header_code").val(html_code).change();
	...
	$(document).ready(function() {
        ...
        var headerEditor = wp.codeEditor.initialize($('#_customize-input-classicstrap_header_code')).codemirror;
        var footerEditor = wp.codeEditor.initialize($('#_customize-input-classicstrap_footer_code')).codemirror;
        ...
        function updateCustomizerOriginalFields() {
            ...
            wp.customize('classicstrap_header_code').set(headerContent);
            wp.customize('classicstrap_footer_code').set(footerContent);
        }
		...
		*/
        ...
        if ( ($("#_customize-input-body_font_object").val().length > 10) && !$("#_customize-input-body_font_object").val().includes('cssImport')) {
            ...
            if (confirm("Your font import settings seem obsolete. Do you want to try to auto-update them? ")) {
                ...
                $("#_customize-input-classicstrap_fonts_header_code").val("");
                ps_update_font_objects_and_import_code(); 
            } 
        }
		...
		$("body").on("click",".cs-toggle-csseditor-position",function(e){
			...
			$('#customize-control-custom_css').toggleClass('classicstrap-maximize-editor');
		});
		...
		$("body").on("click", ".download-scss-vars", function (e) {
			...
			a.href = url;
			a.download = 'classicstrap_bs_variables.json';
			a.click();
