...
function classicstrap_get_css_url (){
    //onboarding: if no CSS custom bundle was created, serve the default one
    if (get_theme_mod("css_bundle_version_number", 0) == 0) return get_stylesheet_directory_uri() . '/'. classicstrap_get_css_optional_subfolder_name() . classicstrap_get_base_css_filename(); 

    //standard case
    return get_stylesheet_directory_uri() . '/' . classicstrap_get_css_optional_subfolder_name() . classicstrap_get_complete_css_filename(); 

}

if (!function_exists('classicstrap_get_css_optional_subfolder_name')):
    function classicstrap_get_css_optional_subfolder_name() { return "css-output/"; }
endif;

if (!function_exists('classicstrap_get_base_css_filename')):
    function classicstrap_get_base_css_filename() { return "bundle.css"; }
endif;

if (!function_exists('classicstrap_get_complete_css_filename')):
    function classicstrap_get_complete_css_filename() { 
        $filename = classicstrap_get_base_css_filename();
...
function classicstrap_get_css_version(){ 
...
add_action( 'wp_enqueue_scripts',  function  () {
    ...
    wp_enqueue_style( 'classicstrap-styles', classicstrap_get_css_url() . '#handlecsserror', array(), classicstrap_get_css_version()); 
...
add_action( 'wp_head', 'classicstrap_add_header_code' );
function classicstrap_add_header_code() {
    if (!get_theme_mod("classicstrap_fonts_header_code_disable")) {
        echo  get_theme_mod("classicstrap_fonts_header_code")." ";
    }
    echo get_theme_mod("classicstrap_header_code");
}

//ADD THE CUSTOM FOOTER CODE (SET IN CUSTOMIZER)
add_action( 'wp_footer', 'classicstrap_add_footer_code' );
function classicstrap_add_footer_code() {
	  //if (!current_user_can('administrator'))
      echo get_theme_mod("classicstrap_footer_code");
}

//ADD THE CUSTOM CHROME COLOR TAG (SET IN CUSTOMIZER)
add_action( 'wp_head', 'classicstrap_add_header_chrome_color' );
function classicstrap_add_header_chrome_color() {
	 if (get_theme_mod('classicstrap_header_chrome_color')!=""):
        ?><meta name="theme-color" content="<?php echo get_theme_mod('classicstrap_header_chrome_color'); ?>" />
	<?php endif;
}

//CSS error handling ENQUEUE: if CSS bundle file is not found, trigger recompile
function classicstrap_add_css_error_handling($url){
...
add_filter( 'clean_url', 'classicstrap_add_css_error_handling', 11, 1 );

//UNRENDER-BLOCK CSS 
// as per https://www.phpied.com/faster-wordpress-rendering-with-3-lines-of-configuration/

function classicstrap_get_headers(){
    //add link to preload CSS bundle
    $headers = "link: <".classicstrap_get_css_url()."?ver=".classicstrap_get_css_version().">; rel=preload; as=style";
...
if(!function_exists('classicstrap_hints')):
    function classicstrap_hints() {  
        header(classicstrap_get_headers());
    }
endif;

if  (!get_theme_mod("disable_bootstrap")) {
    add_action('send_headers', 'classicstrap_hints'); 
}
...
add_action ("template_redirect",function(){
    ...
    echo "<br><br>classicstrap_get_headers:<br><br>". str_replace(",","<br>",htmlentities(classicstrap_get_headers()));
...
function  pico_dequeue_bootstrap() {
    if (get_theme_mod("disable_bootstrap") ):
        // Dequeue styles
        wp_dequeue_style('classicstrap-styles');
